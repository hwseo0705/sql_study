...
-- REPLACE는 실제 데이터를 바꾸지 않고, PROJECT 할때만 보여준다
...
-- 채우지 못해서 공백이 생겨도 CHAR 타입은 글자만 같으면 equal
-- 'SQLD' & 'SQLD  '
-- 하지만, 글자 자체에 공백을 넣어서 비교하면 다르다고 나옴 ????
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
...
-- char_4 > char_6 because D > A --> ASCII: (68 > 65)
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
...
-- VARCHAR2 vs CHAR 일경우, 길이가 다르면 다르고, 공백도 문자로 판단
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
...
-- in order to completely remove spaces, must do update
--UPDATE varchar_compare
--SET varchar_6 = TRIM(vharchar_6)
--WHERE sn = '101';

-- 상수문자열과의 비교
-- CHAR 타입은 공백 제거후 비교, VARCHAR은 그대로 비교
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(varchar_6, ' ', '+') AS varchar_6
FROM varchar_compare
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD' -- 'SQLD  '는 같다고 나온다
;
