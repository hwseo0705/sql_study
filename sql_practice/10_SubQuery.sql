...
FROM tb_emp
WHERE dept_cd IN (
    SELECT -- 복수행 Multi Row
        dept_cd
    FROM tb_emp
    WHERE emp_nm = '이관심')
;

-- 20200525에 받은 급여가 회사전체의 20200525일 전체 평균 급여보다 높은 
-- 사원들의 정보(사번, 이름, 급여지급일, 받은급여액수) 조회
SELECT
...
ORDER BY A.emp_no
; -- GROUP BY, ORDER BY에 명시하지 않은것은 SELECT에 단독으로 나올 수 없음
...
-- 부서원이 2명 이상인 부서 중에서 각 부서의 가장 연장자의 사번, 이름, 생년월일과 부서코드을 조회
...
-- 인라인 뷰 서브쿼리 (FROM 절에 쓰는 서브쿼리)

-- 각 사원의 사번, 이름, 평균 급여정보 조회하고 싶다
...
-- Scala Sub Query : 스칼랏 서브쿼리 (SELECT절에 쓰는 서브쿼리)
-- 사원의 사번, 사원명, 부서명, 생년원일, 성별코드을 조회
