...
-- CROSS JOIN : Cartesian Product (a X b), no join condition, joins everything
SELECT
    *
FROM test_a, test_b
;

-- INNER JOIN (EQUI JOIN) : with join condition, those that match
SELECT
    *
FROM test_a, test_b
WHERE test_a.id = test_b.a_id -- join condition
;
...
SELECT
    A.emp_no, C.emp_nm, B.certi_nm, D.dept_nm
...
SELECT
    B.dept_cd, C.dept_nm, COUNT(A.certi_cd)
...
-- Oracle Join Problem : N tables, N-1 conditions
-- # of join tables increases, the # of join conditions increases
SELECT
    A.emp_no, A.emp_nm, A.addr, A.dept_cd, B.dept_nm
FROM tb_emp A, tb_dept B
WHERE A.addr LIKE '%용인%'
...
-- JOIN ON (ANSI 표준 조인)
-- 1. after FROM before WHERE
-- 2. JOIN keyword 뒤에는 조인할 테이블명을 명시
-- 3. ON 키워드 뒤에는 조인 조건을 명시
-- 4. 조인 조건 서술부(ON)와 일반 조건 서술부 (WHERE)를 분리해서 작성하는 방법
...
-- same as the above but in 'JOIN ON' form
SELECT
    E.emp_no, E.emp_nm, D.dept_nm, C.certi_nm, EC.acqu_de
FROM tb_emp E 
JOIN tb_dept D 
...
-- 2. 즉, 자동으로 2개 이상의 테이블에서 같은 이름을 가진 컬럼을 찾아 INNER 조인을 수행합니다.
-- 3. 이 때 조인되는 동일 이름의 컬럼은 데이터 타입이 같아야 하며, 
--      ALIAS나 테이블명을 컬럼 앞에 표기하면 안됩니다.
...
SELECT
    A.emp_no, A.emp_nm, dept_cd, B.dept_nm -- don't do B.dept_cd cuz that column was used in join
FROM tb_emp A
...
-- # USING
-- 1. NATURAL JOIN에서는 자동으로 이름과 타입이 일치하는모든 컬럼에 대해
--      조인이 일어나지만 USING을 사용하면 원하는 컬럼에 대해서만 선택적 조인조건 부여
