...
-- those on the left that does not match will still print
-- right will be null if no match
-- in inner join it will not print those that don't match but
SELECT 
...
SELECT 
    *
FROM tb_emp A
LEFT JOIN tb_dept B -- LEFT OUTER JOIN or LEFT JOIN
ON A.dept_cd = B.dept_cd
ORDER BY A.emp_no
;
...
WHERE A.dept_cd = B.dept_cd (+) -- oracle left join
ORDER BY A.emp_no
;


-- # RIGHT OUTER JOIN
-- 조인되는 오른쪽 테이블은 모두 조회하고, 왼쪽 테이블은 조건에 매칭되는것만 조회
...
SELECT 
    A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A, tb_dept B
WHERE A.dept_cd (+) = B.dept_cd -- oracle right join
ORDER BY A.emp_no
;
...
SELECT 
    A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm
FROM tb_emp A, tb_dept B
WHERE A.dept_cd (+) = B.dept_cd -- oracle full join
ORDER BY A.emp_no
;
